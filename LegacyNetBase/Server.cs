...
namespace NetBase
{
	public class Server
	{
		...
		public DataReceived OnRequestReceived;
		private Log log;
		...
		private async void ListenerThread()
		{
			while (_isRunning)
			{
				...
				HandeRequest2(req, res);
			}
		}
		private void HandeRequest2(HttpListenerRequest Request, HttpListenerResponse hlresponse)
		{
			#region Setup
			...
			else
			{
				...
				try
				{
					response = OnRequestReceived.Invoke(r);
				}
				catch (Exception ex)
				{
